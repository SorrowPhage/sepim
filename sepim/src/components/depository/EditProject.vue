...
<!--                        <header class="table-heading">新建文件</header>-->
                        <div class="pane-body">
                            <div class="sp-operation-bar">
                                ...
                                <el-button class="sp-space" type="primary" @click="save">修改</el-button>
                            </div>
                            <div class="sp-mavon">
                                <mavon-editor
                                    v-model="md"
...
export default {
    ...
    methods: {
        ...
        save() {
            ...
            axios.post("http://localhost:8080/api/md/update", {
                id:this.$route.query.id, title: this.title, roughly: this.roughly, type: this.value, mdContent: this.mdContent,
                content: this.content, userId: this.$store.state.User.account, username: this.$store.state.User.userName
            }).then(res => {
                if (res.data.flag === "md_edit_succeed") {
                    this.$message({
                        message: '修改成功',
                        showClose: true,
                        type: 'success',
    ...
    created() {
        axios.get("http://localhost:8080/api/md/readMd",{params: {id: this.$route.query.id}}).then(res=>{
